...
def word_embeddings(Y, notes_file, embedding_size, min_count, n_iter):
    ...
    model = w2v.Word2Vec(embedding_size, min_count=min_count, workers=4, iter=n_iter)
...
def fasttext_embeddings(Y, notes_file, embedding_size, min_count, n_iter):
    ...
    model = fasttext.FastText(embedding_size, min_count=min_count, iter=n_iter)
...
def build_pretrain_embedding(embedding_path, word_alphabet, norm):
    ...
    pretrain_emb = np.zeros([len(word_alphabet)+2, embedd_dim], dtype=float32)  # add UNK (last) and PAD (0)
    perfect_match = 0
    case_match = 0
    ...
    pretrain_emb[0, :] = np.zeros([1, embedd_dim], dtype=float32)
    if norm:
